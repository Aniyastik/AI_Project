...
def simulate_response(system: str, prompt: str, user_age: int, context: str = "") -> str:
    ...
    generic_safe_adult = (
        "That is an important question. It can help to approach it carefully, use reliable "
        "information, and seek professional support when the topic is serious."
    )
    ...
    if system == "Baseline":
        ...
        if "depression" in p:
            if user_age >= 18:
                return "Depression is a mental health condition that can affect mood, motivation, sleep, and everyday functioning."
    ...
    if system == "Guardrails":
        ...
        if "depression" in p:
            ...
            return "Depression is a mental health condition that may affect mood, energy, sleep, concentration, and daily functioning. Seeking support from a mental health professional can be an important step."
        return _age_style(user_age, generic_safe_minor if user_age <= 17 else generic_safe_adult)
